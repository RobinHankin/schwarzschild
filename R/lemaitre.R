...
#    for(jj in -1:5){ null_geodesics_lemaitre(jj,-2,TRUE,TRUE,times=seq(from=0,to=1,by=0.01))} # null_geodesics_lemaitre() defined in lemaitre_functions.R
    points(cbind(-1:2,-2),pch=16)


    ## outgoing nulls inside 
    null_geodesics_lemaitre(-1.30,-2,FALSE,TRUE, times=seq(from=0,to=4,by=0.01)) # just barely escapes
    null_geodesics_lemaitre(-1.35,-2,FALSE,TRUE, times=seq(from=0,to=2.83999,by=0.01)) # just barely trapped
    null_geodesics_lemaitre(-1.50,-2,FALSE,TRUE, times=seq(from=0,to=0.72,by=0.01)) # well trapped

    ## lone inward null (this one is on its own, top right):
    null_geodesics_lemaitre(2.50,0,TRUE,FALSE, times=seq(from=0,to=1.396,by=0.01))


    ## Following lines show inward geodesics but due to some numerical
    ## issue they do not extend to large rho so I will omit them.
    ## null_geodesics_lemaitre(0.50,-0.2,TRUE,FALSE, times=seq(from=0.85,to=1.0598,by=0.01))
    ## null_geodesics_lemaitre(0.50,-0.4,TRUE,FALSE, times=seq(from=0.64,to=1.0,by=0.01))
