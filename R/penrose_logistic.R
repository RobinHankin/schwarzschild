penrose_logistic <- function(colours=standard_colours, ...){

  ## plots a Penrose diagram of the whole universe.  

  penrose <- penrose_transform("logistic")

  ## NB: norm looks good and this does not; cf penrose_black_hole
  ...
  for(i in o/10/sqrt(2)){segments(x0=-1.05-i,x1=-1+0.05/2-i,y0=0.55+i,y1=0.6+0.05/2+i,col=colours$photon)}
  ...
  thingvec <- seq(from=-8,to=8,by=0.25)
