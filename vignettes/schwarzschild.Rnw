...
are shown in figure \label{gullstrand_painleve_drops}.  Again we see
...
which also shows white hole and antiuniverse regions.  In this
representation, the singularity appears as a black hyperbola and the
event horizon is a set of $45\circ$ lines.  Ingoing and outgoing null
geodesics are lines at $45^\circ$, which shows that objects inside the
black hole cannot escape.  Figure~\ref{kruskal_szekeres_throw}
also shows worldlines of some moving objects.
