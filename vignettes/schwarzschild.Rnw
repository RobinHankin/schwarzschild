...
(figure~\label{schwarzschild_throw_logx}).  See how outward-directed
...
to one spatial dimension plus time) in a compact region.  A
...
However, here we observe that any function that maps the real line to
the unit interval~$\left(-\pi,\pi\right)$ then
