...
\Abstract{To cite this work, please use~\citet{hankin2021}.
  In the teaching of numerical subjects such as physics,
...
<<results=hide>>=
library("devtools")
devtools::install_github("RobinHankin/schwarzschild")
...
\begin{wrapfigure}[8]{r}{0.2\textwidth}
  \begin{center}
\includegraphics[width=1in]{\Sexpr{system.file("help/figures/schwarzschild.png",package="schwarzschild")}}
  \end{center}
\end{wrapfigure}
...
\begin{figure}[p]
\begin{center}
<<classical,fig=TRUE,height=9,width=9>>=
classical()
@
\caption[Black hole, classical case]{Space near a black
  hole, using\label{classical} classical concepts.  Ingoing and
  outgoing light moves at a constant rate.  Both cross the
  event horizon (fuschia)}
  \end{center}
\end{figure}

\begin{figure}[p]
  \begin{center}
<<classical_drops,fig=TRUE,height=9,width=9>>=
classical(draw_infalling_drops=TRUE)
@
\caption[Black hole, classical case]{Space near a black
  hole, using\label{classical} classical concepts.  Ingoing and
  outgoing light moves at a constant rate.  Both cross th  event
  horizon (fuschia).  World lines of raindrops follow classical
  predictions}
  \end{center}
\end{figure}
