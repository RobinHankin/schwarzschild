...
which is implemented inthe package as function \code{raindrop()}. 

\subsection{Package graphics}

Null- and spacelike geodesics may be plotted in any coordinate system;
figure~\ref{schwarzschild}, in which the unit of distance and of time
is the Schwarzschild radius~$2M$ shows ingoing and outgoing null
geodesics according to equation See how the light cones morph
discontinuously as the event horizon is crossed; also note how the
angle subtended by the inward and outward light approaches zero at the
singularity.  Ingoing null geodesics do not cross the event horizon,
which might be interpreted as saying that nothing passes the event
horizon (see also figure~\ref{schwarzschild_drops} which demonstrates
that ingoing massive particles do not cross the event horizon).
