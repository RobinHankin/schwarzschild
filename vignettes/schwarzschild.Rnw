...
\caption[Kruskal-Szekeres coordinates]{Space near a black hole,
  ...
  the black hole and the singularity unavoidable (the singularity is
  shown as a black hyperbola with non-meaningful grey above).  Lines
  of constant Schwarzschild time are shown in orange and constant
  Schwarzschild radius in green; see how the lines of constant radius
  are spacelike, and lines of constant Schwarzschild time are
  timelike, inside the event horizon}
\end{center}
\end{figure}
...
\caption[Black hole, Kruskal-Szekeres coordinates with free falling
 ...
 \label{kruskal_szekeres_throw} as for figure~\ref{kruskal_szekeres},
  together with the worldlines of a number of freely falling objects}
\end{center}
\end{figure}
...
It is possible to plot contour lines of equal $X,T$ on aSchwarzschild
...
\caption[Space near a black hole, using Lema\^{i}tre
  coordinates]{Space near a black hole, using Lema\^{i}tre
  coordinates~$(\rho,\tau$).  Ingoing light (red) crosses the event
  horizon but outgoing light (blue) does not.  The singularity is
  represented as a $45^\circ$ black line, the gray above being
  meaningless.  The event horizon (fuschia) and the fact that interior
  null geodesics have a slope $>45^\circ$ show that interior light
  cannot escape to infinity.  Infalling raindrops, with zero speed at
  infinity, are shown as vertical dotted lines}
\end{center}
\end{figure}
...
diagram conformally and show, in addition to the universe and the
black hole the antiuniverse and a white hole,
