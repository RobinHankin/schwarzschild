...
the PDF images themselves, this vignette presents a discussion of the
physics underlying the Schwarzschild black hole, as illustrated by the
various diagrams.
...
\noindent and here we consider only the radial line corresponding
to~$\theta=0$, giving
...
\caption[Eddington-Finkelstein outgoing]{Space near a black hole,
  using\label{eddington} Eddington-Finkelstein outgoing coordinates}
\end{center}
\end{figure}
...
@
\caption[Gullstrand-Painleve coordinates]{Space near a black hole,
  using\label{gullstrand_painleve} Gullstrand-Painleve coordinates, with infalling
...
kruskal_with_throw()
@
\caption[Kruskal-Szekeres coordinates with throw]{Space near a black hole,
  using\label{kruskal_szekeres} Kruskal-Szekeres coordinates, together with a thrown
  object}
\end{center}
\end{figure}
...
\begin{figure}[p]
  \begin{center}
<<kruskal_with_throw_plus,fig=TRUE,height=9,width=9>>=
kruskal_with_throw(draw_schwarzschild=TRUE)
@
\caption[Kruskal-Szekeres coordinates with throw plus lines of  constant Schwarzschild~$t$
 and~$r$]{Space near a black hole,  using\label{kruskal_szekeres} Kruskal-Szekeres coordinates,
   together with a thrown object and including lines of constant Schwarzschild~$t$ and~$r$}
\end{center}
\end{figure}
...
kruskal_extended()
@
\caption[Kruskal-Szekeres extended coordinates]{Space near a black hole,
  using\label{kruskal_szekeres} Kruskal-Szekeres coordinates with extension to
  white hole and antiuniverse}
\end{center}
\end{figure}
...
@
\caption[Schwarzschild coordinates, with throw]{Space near a black hole,
  using\label{schwarzshild_throw} Schwarzschild coordinates, together with worldlines of thrown
  objects} 
\end{center}
\end{figure}
...
\caption[Penrose diagram, Cauchy transform]{The universe in compact
  presentation using a \label{penrose_norm} Cauchy Penrose transform}
\end{center}
\end{figure}
