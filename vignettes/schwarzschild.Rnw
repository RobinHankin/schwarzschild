...
\noindent where geometrized units ($G=c=1$) are used and $M$ is the mass of the black hole.  Here we consider only a radial line corresponding to~$\theta, \phi$ constant, giving
...
figures~\ref{schwarzschild} and~\ref{schwarzschild_drops}, in which
the unit of distance and of time is the Schwarzschild radius~$2M$.
...
work might include illustration of the Kerr or Reissner-Nordstr\"{o}m
metrics.
