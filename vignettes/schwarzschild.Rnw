...
\begin{equation}
  ds^2 = -\left(1-2MG/r\right)dt^2 + \frac{dr^2}{1-2MG/r} + r^2d\theta^2+ r^2\sin^2\theta d\phi^2
  \end{equation}

\noindent and here we consider only a radial line corresponding
to~$\theta, \phi$ constant, giving

\begin{equation}
  ds^2 = -\left(1-2MG/r\right)dt^2 + \frac{dr^2}{1-2MG/r}
\end{equation}

These coordinates may be plotted in two dimensional space, as shown in
figures~\ref{schwarzschild} and~\ref{schwarzschild_drops} which use
standard Schwarzschild coordinates with the unit of distance being the
Schwarzschild radius $r_s=2MG$ and the unit of time being $t_s=2MG/c$.
...
objects on the interior of the black hole travels backward in
...
\caption[Black hole, Schwarzschild coordinates, with infalling drops
  ...
  released with zero speed infinity and fall freely.  The world lines
...
\caption[Schwarzschild coordinates, with throw]{Space near a black
  ...
  $r=2r_s=4GM$ and $r=0.9r_s=1.8GM$ respectively.  Objects outside the
...
\caption[Extended Kruskal-Szekeres coordinates]{Space near a black hole,
  ...
  using extended Kruskal-Szekeres coordinates.  The black and white
  holes, and universe and antiuniverse, are labelled}
\end{center}
\end{figure}
...
and we recognise that~$\frac{1}{2} +\frac{\arctan\left(x\pm
  t\right)}{\pi}$ is the distribution function of the Cauchy
  distribution.  This gives a diagram as shown in
  figure~\ref{penrose_cauchy}.  However, the image is visually
  displeasing and it aesthetically preferable to use a logistic or
  Gaussian transform (figures~\ref{penrose_logistic}
...
logistic, and Gaussian transforms, but in each case the event horizon
has an ugly and uninformative shape).
...
\caption[Penrose diagram of universe with a black hole, Cauchy
  ...
  black line) is unavoidable.  Lines of constant Schwarzschild time
  (orange) and constant Schwarzschild radius (green) are shown; see
  how the lines of constant radius are spacelike, and lines of
  constant Schwarzschild time are timelike, inside the event horizon}
\end{center}
\end{figure}
...
diagrams are mathematically accurate which makes the suitable for
educational purposes.  The source code for producing the diagrams is
written in the R programming language and is freely available under
the General Public License.  A variety of coordinate transforms of
Schwarzschild spacetime may be displayed, including Schwarzschild
coordinates, Eddington coordinates, Lema\^{i}tre coordinates,
Gullstrand coordinates, and a number of Penrose transforms.  Further
work might include illustration of the Kerr or Reissner-Nordstr\"{o}
metrics.
\end{document}
